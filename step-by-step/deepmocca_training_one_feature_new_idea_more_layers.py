...
#         self.fc1 = nn.Linear(1719, 1024, bias=False)
...
#         print(adj.to(device).shape)
        ...
        x = self.dropout4(self.bn4(self.fc4(x)))
        return x
...
#         del net
        torch.manual_seed(0)
        net = MyNet(len(proteins), edge_index).to(device)
#         net = tt.practical.MLPVanilla(in_features, [in_features], 1, True, lr_, output_bias=True)
