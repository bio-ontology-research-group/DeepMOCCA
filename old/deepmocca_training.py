...
class MyNet(nn.Module):
    ...
    def forward(self, data):
        ...
        filename = "rank/"+str(sys.argv[1])+"/%s.txt"% name

        with open(filename, 'w') as filehandle:
            for listitem in perm.tolist():
                filehandle.write('%s\n' % listitem)
...
for i in range(len(can_types)):
    ...
    f = open('mapping_files/' + can_types[i] + '.tsv')
    ...
    for l in lines:
        ...
        clinical_file = l[6]
        surv_file = l[2]
        myth_file = data_root + can_types[i] + '/myth/' + l[3]
        diff_myth_file = data_root + can_types[i] + '/diff_myth/' + l[1]
        exp_norm_file = data_root + can_types[i] + '/exp_upper/' + l[-1]
        diff_exp_norm_file = data_root + can_types[i] + '/diff_exp/' + l[0]
        cnv_file = data_root + can_types[i] + '/cnv/' + l[4] + '.txt'
        vcf_file = data_root + can_types[i] + '/vcf/output/' + 'OutputAnnoFile_' + l[5] + '.hg38_multianno.txt.dat'
...
dirFile = 'rank/'+str(sys.argv[1])
search_path = dirFile+ "/*.txt"
allFile = glob.glob(search_path)
for i in range(len(allFile)):
    see = pd.DataFrame(seen.items(), columns=['prot', 'rank'])
    print(allFile[i])
    df = pd.read_csv(allFile[i], header=None, sep='\t')
    df.columns = ['rank']
    df = df.merge(see, on='rank', how='inner')
    df = df[['prot']]
    see = see[['prot']]
    merged = pd.merge(see, df, on='prot', how='outer', indicator=True)
    f = merged.loc[merged['_merge']=='left_only']
    f = f[['prot']]
    df = pd.concat([df, f])
    df.to_csv(allFile[i], sep='\t', index=False, header=False)
